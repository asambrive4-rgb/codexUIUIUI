...
{
    private static readonly byte[] Magic = "CASP1"u8.ToArray();
    private static readonly byte[] Entropy = Encoding.UTF8.GetBytes(
        "CodexAccountSwitcher.ProfileCredential.CurrentUser.v1");
    ...
    public byte[] Unprotect(ReadOnlySpan<byte> protectedPayload)
    {
        ...
        return InvokeDataProtection(
            protectedPayload[Magic.Length..],
            protect: false);
    }

    private static byte[] InvokeDataProtection(
        ...
        bool protect)
    {
        ...
        try
        {
            ...
            var succeeded = protect
                ? CryptProtectData(
                    ref inputBlob,
                    "Codex Account Switcher profile credential",
                    ref entropyBlob,
                    IntPtr.Zero,
                    IntPtr.Zero,
    ...
    {
        var allocationSize = Math.Max(bytes.Length, 1);
        var blob = new DataBlob
        {
            Size = bytes.Length,
            Data = Marshal.AllocHGlobal(allocationSize)
        };
